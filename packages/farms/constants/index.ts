...
export const getFarmConfig = async (chainId: ChainId) => {
  try {
    return (await import(`/${ChainId[chainId].toLowerCase()}.ts`)).default.filter(
...
export const getStableConfig = async (chainId: ChainId) => {
  try {
    const farms = (await import(`/${ChainId[chainId].toLowerCase()}.ts`)).default as SerializedFarmConfig[]
