...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainDefaultId) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainDefaultId, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainDefaultId, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
  ...
  const multicallv3Typed: typeof Multicallv3Typed = async ({ calls, allowFailure, chainId = ChainDefaultId, overrides }) => {
    return multicallv3({
      // @ts-ignore
      calls,
      allowFailure,
  ...
  const multicallv2Typed: typeof Multicallv2Typed = async ({ abi, calls, chainId = ChainDefaultId, options }) => {
    return multicallv2({
      // @ts-ignore
      abi,
      // @ts-ignore
