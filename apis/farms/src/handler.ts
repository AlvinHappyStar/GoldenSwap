...
export async function saveFarms(chainId: number, event: ScheduledEvent | FetchEvent) {
  try {
    ...
    const farmsConfig = await (await fetch(`${farmConfigApi}/${ChainId[chainId].toLowerCase()}.json`)).json<SerializedFarmConfig[]>()
    let lpPriceHelpers: SerializedFarmConfig[] = []
    try {
      lpPriceHelpers = await (
        await fetch(`${farmConfigApi}/priceHelperLps/${ChainId[chainId].toLowerCase()}.json`)
